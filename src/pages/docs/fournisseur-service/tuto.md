...
Vous allez aboutir sur… une page d'erreur. C'est normal ! Enfin non, ce n'est pas normal, c'est plutôt agaçant; ProConnect peut avoir un côté tâtillon.

Ce bug est en cours de correction et avec un peu de chance ne sera plus qu'un souvenir lors de la prochaine mise à jour de ce tuto.

Mais cela nous apprend un élément important: **les codes d'erreur sont un moyen** (encore imparfait, mais nous travaillons à les améliorer) **de vous indiquer ce qu'il faut faire pour remédier à une situation incorrecte**.
...
Si vous obtenez une page indiquant simplement **Bad Request**, c'est probablement que vous n'avez pas ajouté l'URL adéquate. Revenez dans l'Espace Partenaires et vérifiez ce point. L'erreur attendue à ce stade est assortie du code Y000400.

Pour la surmonter, **éditez à la main** la requête générée par OIDC Debugger et supprimez le paramètre response_type. Puis collez la requête modifiée dans votre barre d'adresse.

Vous devriez aboutir sur la page de connexion de ProConnect.
